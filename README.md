...
**Output fields** (existing columns are unchanged; `LABEL` is appended):

- **table**: `IDENTIFIER`, `LOCATION`, `REMOVABLE`, `INTERNAL`, `SIZE`, `NAME` (device/media name), `LABEL` (volume label as shown in Finder and `diskutil list`, e.g. `USBBOOT`)
- **plain**: tab-separated `identifier`, `location`, `size`, `label`
- **json**: top-level `volumeLabel` (plus `volumes[].name` for each partition)

Example table output:

```text
IDENTIFIER  LOCATION  REMOVABLE  INTERNAL  SIZE     NAME              LABEL
disk4       External  true       false     31.0 GB  USB Flash Disk    USBBOOT
```
...
1. **Table (Default)**: Best for terminal readability. `usbfix list` includes a `LABEL` column with the volume name(s) on each disk.
2. **JSON**: Stable and easily parser-friendly, ideal for scripts. `usbfix list` adds a `volumeLabel` field at the disk level:
  ```bash
   usbfix --output json inspect --disk disk4
  ```
3. **Plain**: Emits simple tab-delimited text. `usbfix list` adds the volume label as a fourth field.
